...
shinyUI(fluidPage(withMathJax(),
    ...
    jeden Gast \\( i \\) wird das beobachtete Trinkgeld \\( y_{i} \\) durch den 
    Rechnungsbetrag \\( x_{i} \\) erklärt: $$y_{i}=α+βx_{i}+u_{i}$$ 
    ...
    Ferner wurden normalverteilte Störgrößen \\( u_{i} \\) mit Erwartungswert
    \\( E(u_{i})=0 \\) unterstellt. Die Störgrößenvarianz beträgt \\(
    {\\sigma}^2 = 2 \\), das Siginifikanzniveau \\( a=5\\% \\) und der 
    Beobachtungsumfang (Anzahl der Beobachtungen pro Stichprobe) \\( S=3 \\).
    ...
    fest, wie stark die Störgrößen \\( u_{i} \\) streuen. Wenn Sie die
...
<a href='https://github.com/Oekonometrie-Lernen/Intervallschaetzer' target='_blank'>R
Codes.</a></p>"))),
